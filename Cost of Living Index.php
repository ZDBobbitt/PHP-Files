<?php
/*
 *Template Name: Cost of Living Index
...
d3.csv("https://raw.githubusercontent.com/ZDBobbitt/OccupationSalaries/master/costOfLivingIndex.csv", function(error, data) {
  ...
  data.forEach(function(d) {
    d.rentIndex = +d.rentIndex;
    d.costofLivingIndex = +d.costofLivingIndex;
  });

  x.domain([60, 105]);
  //y.domain(d3.extent(data, function(d) { return d.rentIndex; })).nice();
  y.domain([15, 115]);
  ...
  svg.selectAll(".dot")
    ...
    .enter().append("circle")
      .attr("class", "dot")
      .attr("r", 6)
      .attr("cx", function(d) { return x(d.costofLivingIndex); })
      .attr("cy", function(d) { return y(d.rentIndex); })
      .style("fill", "#49c3ba")
      .style("stroke", "black")
      .on("mouseover", function(d) {
            ...
            div.transition()		
                .duration(200)		
                .style("opacity", .95);		
            div .html("<b>City:</b> " + d.City + "<br/> <b>Rent Index:</b> " + d.rentIndex.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + "<br/> <b>Cost of Living Index:</b> " 
                                     + d.costofLivingIndex.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","))	
